...
def run_demo_case(
    ...
    path = trace_ring_path(a, ring_mask, walkable_mask, clockwise=clockwise)
    ...
    print(
        f"【{label}】环带点数={stats['环带候选点数']}，采样点数={stats['采样点数']}，"
        f"最近点={stats['最近环带点']}，"
...
def main() -> None:
    ...
    cases = [
        {"a": (30, 30), "inner_r": 5, "outer_r": 10, "clockwise": True, "color": (0, 0, 255), "label": "示例一"},
        {"a": (24, 18), "inner_r": 3, "outer_r": 8, "clockwise": False, "color": (255, 0, 0), "label": "示例二"},
    ]
    ...
    for case in cases:
        ...
        all_paths.append((path, case["color"], case["label"]))
    ...
    cv2.putText(output, "示例一=红色(顺时针)", (2, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
    cv2.putText(output, "示例二=蓝色(逆时针)", (2, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
